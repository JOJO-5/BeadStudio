...
- [ ] **Task-023**: 实现图片上传组件（支持拖拽上传 / 支持 PNG JPG WEBP）
- [ ] **Task-024**: 实现图片裁剪功能（集成 react-easy-crop，固定比例裁剪）
- [ ] **Task-025**: 实现图片缩放功能（集成 Pica，支持高质量 Lanczos 插值）
- [ ] **Task-026**: 实现图片像素化预览（实时预览目标分辨率效果）
- [ ] **Task-027**: 将图片处理逻辑封装到 Worker（避免 UI 阻塞）

---

## Phase 4 — 颜色引擎

- [ ] **Task-028**: 实现 PaletteEngine（调色板配置 / 预设调色板 / 自定义调色板）
- [ ] **Task-029**: 实现 ColorMatcher（颜色量化 / LAB 转换 / ΔE2000 计算，集成 image-q）
- [ ] **Task-030**: 实现抖动算法（Floyd-Steinberg / Atkinson / Sierra，可切换）
- [ ] **Task-031**: 实现调色板映射预览（显示原图 → 映射后效果对比）
...
- [ ] **Task-034**: 实现 GridRenderer（画布绘制 bead 网格 / 支持不同 bead 尺寸）
- [ ] **Task-035**: 实现 BoardSplit（自动计算拼板数量 / 板子尺寸 / 排列方式）
- [ ] **Task-036**: 实现 PNG 导出（单板图片 / 全图拼接图片，集成 file-saver）
